...
<section style="height: 400px; position: relative; background-color: #e1e1e1">
    ...
    $requete = $pdo->query('SELECT * FROM users WHERE jeton is null or jeton=""');
    ...
    <table style="height: 80%; background-color: #bdbeef" >
        ...
        <?php foreach ($elements_page as $element) : ?>
            <tr>
                ...
                <td><?= $element['jeton'] ?> Pas de Token valide</td>
                <td><a  style="text-decoration: none; color: #589be7" href="grant_token.php?user_id=<?= $element['user_id'] ?>&email=<?= $element['email'] ?>&nom=<?= $element['nom'] ?>">Délivrer le token</a></td>
